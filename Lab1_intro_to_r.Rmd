...
```{r plot-girls_trend}
ggplot(data=arbuthnot,mapping=aes(x=year, y=girls))+geom_point( )+geom_smooth()
## As we can see from the graph there was a decrease in the numbers of baptism during the 1650s.However, that changed after the 1660s as we can observe a great increase in the amount of girls being baptized.
```
...
## We observe in the graph that there's no profound increase or decrease to the boys born over the years.
```
...
```{r} 
##The data set includes data from 1940 to 2002.The dimensions of the data frame are 63x3 (63 rows and 3 columns). The variable names are year, boys, girls
```
1.  How do these counts compare to Arbuthnot's? Are they of a similar magnitude?
```{r}
## The number of births in the present file are extremely more than in the Arbuthnot. They are definetely not even a bit similar of a magnitude.
```
1.  
...
present <- present %>%
  mutate(boy_ratio = boys / total)
ggplot(data=present,aes(x=year,y=boy_ratio))+geom_point()+geom_smooth(method="lm")
## As we can see in the graph there is a decrease in the birth of boys in the present years so the observation about boys being born in greater proportion than girls doesn't really hold up.
```
...
present %>%
  arrange(desc(total)) %>% slice_max(total)
```##We have the whole row with the maximum total of births in the US and as we can see it was the year 1961
```
